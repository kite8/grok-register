...
const LOCALE_MAP = {
  "app": {
    ...
    "function_state_storage": { title: "功能玩法状态存储", desc: "browser 保存在浏览器本地；server 保存在服务端当前存储后端。" },
...
function buildFieldCard(section, key, val) {
  ...
  else if (section === 'app' && key === 'function_state_storage') {
    built = buildSelectInput(section, key, val, [
      { val: 'browser', text: 'browser / 浏览器' },
      { val: 'server', text: 'server / 服务端' }
    ]);
  }
